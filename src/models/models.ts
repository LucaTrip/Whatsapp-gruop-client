export interface MyDefaultEventsMap {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  [event: string]: (...args: any[]) => void;
}

export interface IWaInfo {
  qr: string;
}

export interface IKrossReservationList {
  count: number;
  data: IKrossReservationUser[];
  limit: number;
  offset: number;
  result: boolean;
  total_count: number;
...
export interface IFavouriteMember {
  fullName: string;
  phoneNumber: string;
}
